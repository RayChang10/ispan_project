...
class EdgeTTS(BaseTTS):
    def txt_to_audio(self,msg):
        ...
        # 新增：從設定讀取語速/音高/音量（皆為字串）
        rate = getattr(self.opt, "EDGE_RATE", "+20%")     # 例："+15%"、"-10%"
        pitch = getattr(self.opt, "EDGE_PITCH", "+0Hz")   # 例："+4Hz"、"-3Hz"
        volume = getattr(self.opt, "EDGE_VOLUME", "+0%")   # 例："+0%"

        t = time.time()
        asyncio.new_event_loop().run_until_complete(self.__main(voicename,text, rate, pitch, volume))
    ...
    async def __main(self, voicename: str, text: str,
        ...
        try:
            communicate = edge_tts.Communicate(
                text, voicename, rate=rate, pitch=pitch, volume=volume
            )
            first = True
